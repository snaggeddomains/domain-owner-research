...
TLD DEMAND: ${s.tld_demand ? `registered in ${s.tld_demand.count} TLDs (${s.tld_demand.band} demand) — more extensions taken = more proven commercial demand for the word` : 'not checked'}
RENEWAL COST (if we acquired it): ${s.renewal_cost && s.renewal_cost.renewal ? `~$${s.renewal_cost.renewal}/yr${s.renewal_cost.premium ? ` — ⚠️ REGISTRY-PREMIUM (~${s.renewal_cost.multiple || '?'}× standard), a real recurring holding cost that eats resale margin` : ' (standard)'}` : 'unknown'}
