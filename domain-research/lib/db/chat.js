...
export async function appendChat(runId, domain, role, content) {
  if (!isDbConfigured() || !runId || !content) return null;
  try {
    const { data, error } = await getDb()
      .from(TABLE)
      .insert({ run_id: runId, domain: domain || null, role, content: String(content).slice(0, 12000) })
      .select('id')
      .single();
