...
import { registrarContactFor } from './registrarContact.js';
import * as openai from './llm/openai.js';
import * as anthropic from './llm/anthropic.js';

// Marketplace / parking nameservers → the domain is PARKED FOR SALE via that channel. This is
// AFFIRMATIVE for-sale evidence even when marketplace_check/domainscout returned no catalog listing
// (their scans miss JS/lander-only listings). Suffix-matched against the domain's resolved NS.
const MARKETPLACE_NS = [
  [/(^|\.)afternic\.com$/i, 'Afternic', (d) => `https://www.afternic.com/domain/${d}`],
  [/(^|\.)above\.com$/i, 'Afternic', (d) => `https://www.afternic.com/domain/${d}`],
  [/(^|\.)dan\.com$/i, 'Dan', (d) => `https://dan.com/buy-domain/${d}`],
  [/(^|\.)undeveloped\.com$/i, 'Dan', (d) => `https://dan.com/buy-domain/${d}`],
  [/(^|\.)sedoparking\.com$/i, 'Sedo', (d) => `https://sedo.com/search/details/?domain=${d}`],
  [/(^|\.)sedo\.com$/i, 'Sedo', (d) => `https://sedo.com/search/details/?domain=${d}`],
  [/(^|\.)hugedomains\.com$/i, 'HugeDomains', (d) => `https://www.hugedomains.com/domain_profile.cfm?d=${d}`],
  [/(^|\.)sav\.com$/i, 'Sav', (d) => `https://www.sav.com/${d}`],
  [/(^|\.)squadhelp\.com$/i, 'Atom (Squadhelp)', (d) => `https://www.atom.com/name/${d}`],
  [/(^|\.)atom\.com$/i, 'Atom', (d) => `https://www.atom.com/name/${d}`],
];
function marketplaceFromNs(nameservers, domain) {
  for (const ns of nameservers || []) {
    const host = String(ns || '').toLowerCase();
    for (const [re, name, link] of MARKETPLACE_NS) if (re.test(host)) return { name, link: link(domain), ns: host };
  }
  return null;
}
...
- RULE — enrich any candidate owner: as soon as you have even moderate confidence in a potential owner (a real person's name, organization, email or phone from WHOIS/RDAP/site/archive/cluster/trademark — anything), run what you have through rocketreach_search (it is FREE and spends no credits) to find additional professional context (current employer, title, LinkedIn, location). Do this on the free pre-flight pass too. Then, on the deep pass, run rocketreach_lookup (premium) to pull actual EMAIL and PHONE for EVERY named person you have a LinkedIn URL or name+company for — not only the single "primary owner". fullenrich_lookup is a COST-GATED FALLBACK, NOT a parallel corroborator — it spends PREMIUM credits and PHONE numbers cost FAR more than emails, so use it sparingly. Run it for a named person ONLY when RocketReach did NOT yield a usable contact for them: a rocketreach_search that returns ZERO profiles, or a rocketreach_lookup that returns no email/phone, is the trigger (its whole value is finding contacts RocketReach's database LACKS — e.g. a "domain investor" RocketReach has no record of). Do NOT run it when rocketreach_lookup ALREADY returned a deliverable email/phone for that person — that just double-spends. fullenrich_lookup is EMAILS ONLY — you CANNOT request a phone number from it (the include_phone option is not exposed to you). FullEnrich's phone waterfall is the single most expensive call we make (~$1.50 each), so it is NEVER run automatically during research; if the user later needs a direct phone for the primary owner they pull it on demand via the "Get phone number" button in the report. Do not attempt or mention a FullEnrich phone lookup here for anyone — just get the email. Pass the candidate's NAME plus an anchor (the domain under research, their company, OR a LinkedIn URL). When both providers do end up with data, the report consolidates/de-dupes overlapping emails/phones into one clean block. Like RocketReach, NEVER run fullenrich_lookup on a broker/marketplace platform. This explicitly INCLUDES a reachable employee or contact at the owning company (e.g. a "Business Development Manager" you found via LinkedIn/rocketreach_search): look them up too, since that human is often the practical way to reach the holder. When you have a LinkedIn URL, pass it as linkedin_url (most reliable); otherwise name+company. Whenever rocketreach_search surfaces a person's LinkedIn, that is a signal to immediately rocketreach_lookup them. A search returning no profiles, or a lookup with no emails, just means RocketReach has no record (not that contact info doesn't exist) — fall back to the owner's active entity or the registrar contact form. NEVER enrich a marketplace/broker PLATFORM or its staff (Atom, GoDaddy/Afternic, Dan, Sedo, Sav, Brannans/Zito, etc.) — those are standard channels, not the owner; do not look up a broker's LinkedIn/email/phone.
...
- CERTIFICATE TRANSPARENCY (cert_transparency — FREE, runs on any pass): every TLS cert a domain has used is public, and you cannot privacy-wall a cert. It returns the domain's own SUBDOMAINS (what services they actually run) and, crucially, OTHER registrable domains that shared a certificate with the target (related_domains) — a strong same-operator signal. When the target is privacy-shielded, check cert_transparency for a cert-sibling that DOES have public WHOIS/contact and pivot to it (whois_lookup / rdap_whois / web_search), same as the nameserver-sibling play.
- EMAIL INFRASTRUCTURE (email_infra — FREE): parses the target's MX/SPF/DMARC into ownership hints. The DMARC rua/ruf REPORTING ADDRESSES and non-generic SPF include: hosts frequently name the operating company or a real contact domain even under WHOIS privacy — pursue operator_domain_hints (whois_lookup / web_search the domain) and treat a distinctive DMARC mailbox as a lead. It also flags marketplace verification TXT tokens tying the domain to a seller account.
- REVERSE-ANALYTICS (reverse_analytics — paid, deep pass, needs a key): the counterpart to analytics_footprint. Once analytics_footprint EXTRACTS a tracking id (GA4 G-, UA-, GTM-, AdSense ca-pub-, Meta pixel) from the target or its lander, run reverse_analytics on that id to enumerate EVERY other site carrying it — the operator's portfolio. Then pivot to a sibling with public ownership info. This is decisive same-owner clustering (a shared PLATFORM id only proves same platform — prefer a distinctive GA/AdSense id).
- CORPORATE REGISTRY (opencorporates_search — deep pass, needs a key): when the owner resolves to a COMPANY (a registrant org, an operating LLC, a redirect target's parent), search it here to get OFFICERS (named directors) + the registered address, then enrich those people (rocketreach_lookup / web_search). Turns "owned by Acme LLC" into reachable humans.
- COMPANY EMAIL PATTERN (hunter_search — deep pass, needs a key): for a company domain, returns the email PATTERN (e.g. {first}.{last}@) + known verified addresses — use it to construct/confirm a reachable email for a named owner when rocketreach_lookup / fullenrich_lookup came up empty.
...
- NAMEPROS (namepros_search) — the main domain-investor/trader FORUM, often the fastest way to name a private seller. Run namepros_search on the EXACT domain WHENEVER it looks aftermarket / investor-held / for-sale (privacy WHOIS, marketplace nameservers, a marketplace_check hit, or a known investor like a "domain investor" surfaced from WHOIS/press), OR whenever you already have a candidate name/handle. WTS (sale), WTB, auction, appraisal and discussion threads frequently NAME the seller — a NamePros handle, sometimes a real name — plus the asking price, whether it SOLD, and a contact. read_url the most relevant threads to extract those, then PIVOT a NamePros handle to a real identity: open the member's NamePros profile/signature, web_search/brave_search the handle (and handle + "domains"/"portfolio"), and cross-reference any portfolio brand. Also namepros_search a candidate NAME to find that trader's own threads/sales. CAVEAT: a NamePros thread is community chatter, NOT a live marketplace listing — only state the domain is "for sale" if the thread explicitly says so AND is current; treat a years-old "sold" thread as a historical ownership-movement clue, not the current status.
...
function deriveTooling(env, tier) {
  ...
  const preRun = ['masterlist_lookup', 'universe_ownership', 'domainiq_lookup', 'ns_siblings', 'cert_transparency', 'email_infra'];
...
export async function gather({ domain, question, history = [], env, tier = 'all', lessons = '', chatCorrections = '', userNotes = '' }) {
  ...
  // Registrant-contact channel — resolve the domain's registrar (deterministic
  // RDAP/WHOIS) to its ICANN registrant-contact relay form / privacy-proxy email
  // so a privacy-walled report routes the owner contact to the PRECISE form, not a
  // generic registrar homepage. Fail-open: any error just omits the seed.
  try {
    const w = await whoisLookup(domain).catch(() => null);
    const reg = w && w.registrar;
    const rc = reg && (reg.name || reg.ianaId) ? registrarContactFor(reg, domain) : null;
    if (rc && (rc.url || rc.email)) {
      seedTrace.push({ tool: 'registrar_contact', args: { domain }, ok: true, error: null, data: JSON.stringify({ registrar: reg.name, ianaId: reg.ianaId, ...rc }).slice(0, 600) });
      const channel = [rc.url && `contact form ${rc.url}`, rc.email && `privacy-relay email ${rc.email}`].filter(Boolean).join(' · ');
      seedParts.push(
        `REGISTRANT-CONTACT CHANNEL — the domain's registrar of record is ${reg.name || 'the resolved registrar'}${reg.ianaId ? ` (IANA ${reg.ianaId})` : ''}, whose ICANN-mandated WHOIS-relay / registrant-contact channel is: ${channel}. A message sent through it is forwarded to the CONCEALED registrant behind WHOIS privacy — it is the authoritative public way to reach the hidden owner. RULE: whenever you do NOT have a solid DIRECT lead on the real owner (registrant privacy-shielded, owner_type marketplace_only, or confidence Medium/Low), you MUST include this EXACT relay in "contact_path" as a concrete step to reach the concealed registrant, and cite it in the report's Recommended Contact Path — use THIS precise URL/email verbatim, never a generic registrar homepage or support page. If you DO confidently identify the real owner with a direct contact, keep this only as a secondary fallback route.`
      );
    }
    // Marketplace-nameserver for-sale signal — a domain resolving to a marketplace's parking NS
    // (Afternic/Sedo/Dan/HugeDomains/Sav/Atom) is PARKED FOR SALE via that channel. Deterministic +
    // authoritative — overrides a "not-listed" marketplace scan (which misses lander-only listings).
    const mkt = w && marketplaceFromNs(w.nameservers, domain);
    if (mkt) {
      seedTrace.push({ tool: 'marketplace_ns', args: { domain }, ok: true, error: null, data: JSON.stringify(mkt).slice(0, 300) });
      seedParts.push(
        `FOR-SALE SIGNAL (nameservers) — ${domain}'s nameservers point to ${mkt.name}'s parking/sales service (${(w.nameservers || []).join(', ')}). A domain on a marketplace's parking nameservers is PARKED / LISTED FOR SALE via ${mkt.name}${mkt.link ? ` (${mkt.link})` : ''} — this is AFFIRMATIVE for-sale evidence. Treat the domain as FOR SALE via ${mkt.name} even if marketplace_check / domainscout_lookup returned NO catalog listing (those scans miss JS-rendered / lander-only listings). Do NOT describe it as merely "held" or "not for sale" — report it as for sale via ${mkt.name} and route the contact path through the listing plus the registrar's WHOIS-relay form (the underlying owner stays privacy-shielded, so identity confidence is Medium/Low unless you independently name them).`
      );
    }
  } catch { /* fail-open — no registrar-contact / marketplace-NS seed */ }
