...
- RULE — enrich any candidate owner: as soon as you have even moderate confidence in a potential owner (a real person's name, organization, email or phone from WHOIS/RDAP/site/archive/cluster/trademark — anything), run what you have through rocketreach_search (it is FREE and spends no credits) to find additional professional context (current employer, title, LinkedIn, location). Do this on the free pre-flight pass too. Then, on the deep pass, run rocketreach_lookup (premium) to pull actual EMAIL and PHONE for EVERY named person you have a LinkedIn URL or name+company for — not only the single "primary owner". For the PRIMARY likely owner (and each key named contact), ALWAYS ALSO run fullenrich_lookup — a multi-provider WATERFALL that returns WORK + PERSONAL emails (each with a deliverability status) and PHONE. This is MANDATORY on the paid pass and runs IN PARALLEL with rocketreach_lookup: call BOTH for the primary owner, and do NOT skip FullEnrich even when rocketreach_lookup ALREADY returned emails/phones — the second provider corroborates the hit (agreement = high-confidence) and routinely adds a personal email or mobile RocketReach lacks. Run it as soon as you have a candidate's NAME plus an anchor (the domain under research, their company, OR a LinkedIn URL), INDEPENDENTLY of RocketReach's outcome. Do NOT gate it on rocketreach_search returning a profile or on rocketreach_lookup running: FullEnrich's whole value is finding contacts RocketReach's database LACKS, so a rocketreach_search that returns ZERO profiles for a named candidate is itself a REASON to run fullenrich_lookup (pass name + the domain under research — e.g. a "domain investor" RocketReach has no record of). When both providers return data they corroborate each other (agreement = high-confidence) and the report consolidates/de-dupes overlapping emails/phones into one clean block. Like RocketReach, NEVER run fullenrich_lookup on a broker/marketplace platform. This explicitly INCLUDES a reachable employee or contact at the owning company (e.g. a "Business Development Manager" you found via LinkedIn/rocketreach_search): look them up too, since that human is often the practical way to reach the holder. When you have a LinkedIn URL, pass it as linkedin_url (most reliable); otherwise name+company. Whenever rocketreach_search surfaces a person's LinkedIn, that is a signal to immediately rocketreach_lookup them. A search returning no profiles, or a lookup with no emails, just means RocketReach has no record (not that contact info doesn't exist) — fall back to the owner's active entity or the registrar contact form. NEVER enrich a marketplace/broker PLATFORM or its staff (Atom, GoDaddy/Afternic, Dan, Sedo, Sav, Brannans/Zito, etc.) — those are standard channels, not the owner; do not look up a broker's LinkedIn/email/phone.
