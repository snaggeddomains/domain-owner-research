...
- INVESTOR-PORTFOLIO PIVOT (works even when WHOIS is private and the listing is anonymized): a marketplace listing (Atom/Afternic/Dan/Sedo) hides the seller, but many domain investors run their OWN branded portfolio/marketplace that mirrors those listings and names them. web_search the domain itself; if a seller-branded site lists it (e.g. a personal domain-marketplace landing like "domainman.com/name/<domain>"), pivot that portfolio brand to the operator's real NAME via the open web (Quora, LinkedIn, About/press pages), then to their LinkedIn profile, location and employer. This is often the fastest path to a private owner.
- EMAIL INFERENCE + VERIFICATION: with a distinctive owner NAME but no published email, infer the most likely personal address(es) (first.last@gmail.com, first@lastname.com, etc.) and VERIFY by running each through whoxy_reverse (email=…). A small, coherent result set — especially one whose sibling domain cross-checks back to the same seller/portfolio — confirms BOTH the email and the owner. Only present a guessed email as confirmed when corroborated this way; otherwise label it "inferred, unverified".
