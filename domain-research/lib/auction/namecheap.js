...
const HANDLE_STOP = new Set(['the', 'this', 'sale', 'market', 'namecheap', 'seller', 'buyer', 'now', 'here', 'our', 'your']);

// Pull the winning handle from rendered Namecheap sale text. The page shows the
// price then "from <handle>" (the winner), and a Bid History whose top row is the
// same handle. We take the handle in the "from <handle>" phrase.
function parseWinner(text) {
  const t = String(text || '');
  const sold = /\bSOLD\b/i.test(t) || /Ended on /i.test(t);
  // "$1,275 from keepquiet" — the price-adjacent "from <handle>" is the winner.
  let handle = null;
  const m = t.match(/\$[\d,]+\s*(?:USD)?\s*from\s+([A-Za-z0-9._-]{2,40})/i)
    || t.match(/\bfrom\s+([A-Za-z0-9._-]{2,40})\b/i);
  if (m) {
    const h = m[1].trim();
    if (HANDLE_RE.test(h) && !HANDLE_STOP.has(h.toLowerCase())) handle = h;
  }
  return { handle, sold };
}
...
export async function detectNamecheapAuction(domain, env = {}) {
  ...
  try {
    ...
    const { handle, sold } = parseWinner(text);
    if (!handle) return null;
    return { handle, sold: !!sold, sale_url: saleUrl };
  } catch {
    return null;
  }
}
