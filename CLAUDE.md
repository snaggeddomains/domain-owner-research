...
9. **Every new module/submodule MUST be uniformly navigable + ⌘K-enabled from ANYWHERE (Rob, 2026-09-01).** When you add a module or submodule (a new tool, report, tab, or standalone page), it is not done until you've verified it's reachable via the ⌘K palette from BOTH apps — the research SPA (`public/app.js`: a `.nav-btn` in its section group so the DOM-scan finds it, OR an entry in `CMDK_CROSS_APP` for a cross-app/hidden destination — use `section:'always'` for a universal page) AND the admin app (`snagged-admin app/command-palette.tsx`: put it in a nav tab array in `lib/permissions.ts` — auto-covered — or in that palette's `extraDests()` if it's a standalone/universal page). Same permission gating as the page itself. Keep `CMDK_CROSS_APP` (research) in sync with the admin nav tab arrays. Treat this as part of shipping the feature, like the memory note — a module you can't ⌘K-to from every section isn't finished.
