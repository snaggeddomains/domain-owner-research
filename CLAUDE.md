...
8. **ALWAYS deep-link SQL to run on GitHub (Rob, 2026-08-28).** Whenever a change needs Rob (or an admin) to run SQL / a migration, give the GitHub deep-link to the file, not just a path — `https://github.com/snaggeddomains/domain-owner-research/blob/main/<repo-relative-path>` (admin repo: `https://github.com/snaggeddomains/snagged-admin/blob/main/<path>`). Commit + push the SQL file FIRST so the `main` link resolves. (Same rule in the admin repo.)
