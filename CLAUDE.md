...
9. **Every new module/submodule MUST be uniformly navigable + ⌘K-enabled from ANYWHERE (Rob, 2026-09-01).** When you add a module or submodule (a new tool, report, tab, or standalone page), it is not done until you've verified it's reachable via the ⌘K palette from BOTH apps — the research SPA (`public/app.js`: a `.nav-btn` in its section group so the DOM-scan finds it, OR an entry in `CMDK_CROSS_APP` for a cross-app/hidden destination — use `section:'always'` for a universal page) AND the admin app (`snagged-admin app/command-palette.tsx`: put it in a nav tab array in `lib/permissions.ts` — auto-covered — or in that palette's `extraDests()` if it's a standalone/universal page). Same permission gating as the page itself. Keep `CMDK_CROSS_APP` (research) in sync with the admin nav tab arrays. **AND it must carry the STANDARD HEADER (Rob, 2026-09-01):** every new module/submodule/standalone page — research OR admin — must show the same top-of-page header controls as every other module: the **back / refresh / share** icons, the **💡 Feedback** link, the **🔔 notifications** bell, and the **account avatar** (plus the section switcher). Research SPA pages render inside the shared `.topbar` (PWA back/refresh/share + 💡 + bell + avatar) — a standalone/hidden view must still show it. (The **💡 Feedback** link `#topbar-feedback` in `index.html`'s `.topbar__account` → `/feedback`, universal like the admin TopBar's — added 2026-09-02 after it was missing on the research header.) Admin pages get it from `SectionChrome`/`TopBar` automatically; a standalone admin page (its own `<TopBar>`, like `/feedback`) needs `NavControls` extended to its path. Treat this as part of shipping the feature, like the memory note — a module you can't ⌘K-to from every section, that isn't on the menus, OR that's missing the standard header icons isn't finished.
8. **ALWAYS deep-link SQL to run AND Actions to dispatch, on GitHub — and NAME THE EXACT PROJECT (Rob, 2026-08-28).** Whenever a change needs Rob (or an admin) to run SQL / a migration OR dispatch a GitHub Action, give the GitHub deep-link, not just a path/name. **For SQL, ALWAYS state the EXACT Supabase project by its project-PICKER name** (the name shown in the Supabase project dropdown), never a vague "main/research". The projects are: **`domain-owner-research`** (the main/research project, marked PRODUCTION — holds ALL `domain_research_*` tables incl. `deals` / `owner_review_cards` / `feature_requests`; this is `SUPABASE_URL` for BOTH apps), **`snagged-naming-universe`** (naming — `name_universe`, `english_words`), **`Master Domain Name List`** (master), **`snagged-zone-index`** (zone). (There's also a `rob-personal` project in the picker — we NEVER use it for these tables.) It's easy to run in the wrong one (0022 got run in snagged-naming-universe instead of domain-owner-research once → tables in the wrong project, nothing populated). So say e.g. "run on the **`domain-owner-research`** project (the PRODUCTION one with the other `domain_research_*` tables — NOT snagged-naming-universe)". SQL file: `https://github.com/snaggeddomains/domain-owner-research/blob/main/<repo-relative-path>`. Action: `https://github.com/snaggeddomains/domain-owner-research/actions/workflows/<file>.yml` (links straight to Run-workflow). Admin repo: swap the base to `.../snagged-admin/…`. This extends to **any repo file/artifact** Rob needs to open/run/review on GitHub (a script, config, migration, PR, `.../commit/<sha>`) — default to a clickable GitHub URL, never a bare path/name. Commit + push the file FIRST so the `main` link resolves. (Same rule in the admin repo.)
...
- **Cross-app Reports tabs in the palette (2026-07-24).** The palette found nothing for e.g.
  ...
  - **Full uniform-coverage audit (2026-09-01).** Swept every module/submodule so the palette reaches
    ALL of them from anywhere (Rob's standing rule #9). Added to `CMDK_CROSS_APP`: **Reports · Ahrefs Report**
    (`/research/ahrefs`), **Deals · My Tasks** (`/deals/tasks`), **Deals · Owner Review** (`/deals/owner-review`),
    **SNAP · SNAP Opportunities** (`/reports/opportunities`), **SNAP · SNAP Names** (`/reports/snap-names`), and a
    **universal Feedback** entry (`section:'always'`, no section gate — the loop special-cases `always`). The
